# HaikalaC — Claude project instructions

## Token / Cost Ledger (billing-grade)

The owner bills clients from `LEDGER.md` at this repo's root (generated/updated by `~/.claude/billing/ledger.py`). Entries must be **exact, not estimated**.
...
- **Keep pricing current.** If you notice Anthropic pricing has changed, stop and notify the owner (or update `~/.claude/billing/pricing.json` in the billing tool’s repository) before running the ledger again. Past rows correctly preserve historical pricing.
